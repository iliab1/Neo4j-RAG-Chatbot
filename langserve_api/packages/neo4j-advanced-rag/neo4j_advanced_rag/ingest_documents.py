...
# Define a callback handler for output
...
def ingest_documents(file_path: Path):
    ...
    for i, parent in enumerate(parent_documents):
        questions = question_chain.invoke(parent.page_content, config=config).questions
        params = {
            "parent_id": i,
            "questions": [
    ...
    # Ingest summaries
    for i, parent in enumerate(parent_documents):
        summary = summary_chain.invoke({"question": parent.page_content}, config=config).content
        params = {
            "parent_id": i,
            "summary": summary,
